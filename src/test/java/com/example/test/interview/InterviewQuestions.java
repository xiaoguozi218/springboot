 ...
 *
 *  3、TCP 的可靠性到底指的是什么？
 *     1、三次握手机制 - 三次握手机制在一定程度上保证了数据的可靠传输，两端在建立连接时，通过请求--同步请求应答--确认应答 三个步骤来确认两端同步知晓建立连接；
 *     2、重传机制 - 由于网络抖动或不稳定等问题造成数据包丢失，在一定的时间段对端没有收到该数据包时，会告知发送端重传该数据包，保证了数据的完整性；
 *     3、数据校验 - TCP首部有个字段是一个字段就是校验和，通过将伪首部、TCP首部以及数据使用累加和校验出一个数字放入校验和字段里，接收端再通过同样的算法去核对校验和是否正确，保证了数据的安全性；
 *     4、拥塞控制 - 防止过多的数据传入网络中造成网络设备或链路的负载，保证了数据可以稳定、安全、快速的传送至对端；
 *
 *《操作系统》-
 *  1、僵尸进程和孤儿进程是什么(区别)
 *     - 僵尸进程：父进程创建出子进程，子进程退出了，父进程没有调用wait或waitId获取子进程的信息(状态)，子进程的描述符仍在系统中。
 *       僵尸进程危害：1、系统进程表是一项有限资源，如果系统进程表被僵尸进程耗尽的话，系统就可能无法创建新的进程。
