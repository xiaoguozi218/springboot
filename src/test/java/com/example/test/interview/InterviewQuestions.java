 ...
 *  4、TCP 三次握手和四次挥手(面试常客) - 为了准确无误地把数据送达目标处，TCP协议采用了三次握手策略。
 *     - 三次握手：1）客户端 –>发送带有 SYN 标志的数据包 – 一次握手 –> 服务端
 *               2）服务端 –>发送带有 SYN/ACK 标志的数据包 – 二次握手 –> 客户端
 *               3) 客户端 –> 发送带有带有 ACK 标志的数据包 – 三次握手 –> 服务端
 *       - 为什么要三次握手? - 三次握手的目的是建立 可靠的通信信道,就是双方确认自己与对方的 发送与接收 是正常的。
 *       - 为什么要传回 SYN? - 接收端传回发送端所发送的 SYN 是为了告诉发送端，我接收到的信息确实就是你所发送的信号了。
 *     - 四次挥手: 断开一个 TCP 连接则需要“四次挥手”：
 *              1) 客户端-发送一个 FIN，用来关闭客户端到服务器的数据传送
 *              2) 服务器-收到这个 FIN，它发回一 个 ACK，确认序号为收到的序号加1 。和 SYN 一样，一个 FIN 将占用一个序号
 *              3) 服务器-关闭与客户端的连接，发送一个FIN给客户端
 *              4) 客户端-发回 ACK 报文确认，并将确认序号设置为收到序号加1
 *       - 为什么要四次挥手? - 任何一方都可以在数据传送结束后发出连接释放的通知，待对方确认后进入半关闭状态。
 ...
 *  5、TCP、UDP 协议的区别：
 *     - TCP：TCP提供面向连接的服务。在传送数据之前必须先建立连接，数据传送结束后要释放连接。
 *           TCP 不提供广播或多播服务。由于 TCP 要提供可靠的，面向连接的运输服务（TCP的可靠体现在TCP在传递数据之前，会有三次握手来建立连接，而且在数据传递时，有确认、窗口、重传、拥塞控制机制，
 *           在数据传完后，还会断开连接用来节约系统资源），这一难以避免增加了许多开销，如确认，流量控制，计时器以及连接管理等。这不仅使协议数据单元的首部增大很多，还要占用许多处理机资源。
 *           TCP 一般用于文件传输、发送和接收邮件、远程登录等场景。
 *     - UDP：UDP 在传送数据之前不需要先建立连接，远地主机在收到 UDP 报文后，不需要给出任何确认。
 *           虽然 UDP 不提供可靠交付，但在某些情况下 UDP 确是一种最有效的工作方式（一般用于即时通信），比如： QQ 语音、 QQ 视频 、直播等等
 *
 *  6、什么是长连接，什么是短连接？长连接和短连接的区别是什么？
 *     - 长连接：所谓长连接，指在一个TCP连接上可以连续发送多个数据包，在TCP连接保持期间，如果没有数据包发送，需要双方检测包以维持此连接，一般需要自己做在线维持。
 *     - 短连接：是指通信双方有数据交互时，就建立一个TCP连接，数据发送完成后，则断开此TCP连接，一般银行都是用短连接。
 *     - 区别：
 *     - 注意：HTTP协议的长连接和短连接，实质上是TCP协议的长连接和短连接。
 *
 *《操作系统》-
 *  1、僵尸进程和孤儿进程是什么(区别)
 ...
 *  2、
 *
 *  3、有的同学对于Object obj = new Object();这样的操作在多线程的情况下会拿到一个未初始化的对象这点可能有疑惑，这里也做个简单的说明。以上java语句分为4个步骤：
 *     - 在栈中分配一片空间给obj引用
 *     - 在jvm堆中创建一个Object对象，注意这里仅仅是分配空间，没有调用构造方法
 *     - 初始化第2步创建的对象，也就是调用其构造方法
 *     - 栈中的obj指向堆中的对象
 *
 *
 */
