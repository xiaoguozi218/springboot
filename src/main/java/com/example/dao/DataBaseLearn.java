 ...
 *  1、名次解释：- B tree： B树属于多叉树又名 平衡多路查找树。
 *             - B-tree：B-树就是B树,B树（B-Tree，并不是B“减”树，横杠为连接符，容易被误导）B树属于多叉树又名 平衡多路查找树。每个节点可以多个数（由磁盘大小决定）。
 ...
 *  总结：今天，我们讲解了数据库索引实现，依赖的底层数据结构，B+ 树。它通过存储在磁盘的多叉树结构，做到了时间、空间的平衡，既保证了执行效率，又节省了内存。
 *      我这里再总结一下 B+ 树的特点：（m叉树）
 ...
 *      - B 树中的叶子节点并不需要链表来串联。
 *  也就是说，B 树只是一个每个节点的子节点个数不能小于 m/2 的m叉树。
 *
 *
 *
 ...
 *     - 核心问题：（1）旧版本数据存储在哪里？- 旧版本数据存储在回滚段里；
 *   * - InnoDB为何能够做到这么高的并发？
 *       - 回滚段里的数据，其实是历史数据的快照（snapshot），这些数据是不会被修改，select可以肆无忌惮的并发读取他们。
 *       - 快照读（Snapshot Read），这种一致性不加锁的读（Consistent Nonlocking Read），就是InnoDB并发如此之高的核心原因之一。
 *       - 什么样的select是快照读？- 除非显示加锁，普通的select语句都是快照读，例如：select * from t where id>2;
 *       - 这里的显示加锁，非快照读是指：select * from t where id>2 lock in share mode; 或
