 ...
 * @Auther: gsh
 * @Date: 2018/8/8 14:29
 * @Description:    4.2 Asynchronous networking without Netty
 */
public class PlainNioServer {
    ...
    public void serve(int port) throws IOException {
        ...
        serverChannel.register(selector, SelectionKey.OP_ACCEPT);    //3-注册 ServerSocket 到 ServerSocket ，并指定这是专门用于接受 连接。
        ...
        for (;;) {
            ...
            while (iterator.hasNext()) {
                ...
                iterator.remove();
                try {
                    if (key.isAcceptable()) {                //6-检查该事件是一个新的连接，准备好接受。
