 ...
 *
 *《JVM 垃圾回收机制 - GC》:
 *  1、垃圾回收器的种类 ：大概可以分成 四种 。- 但是Java 11包含一个全新的垃圾收集器--ZGC，TB级的堆随便搞！暂停时间10毫秒内！吞吐量最大下降15%！
 *      1、串行垃圾回收器（Serial Garbage Collector）
 *      2、并行垃圾回收器（Parallel Garbage Collector）
 *      3、并发标记扫描垃圾回收器（CMS Garbage Collector）- CMS 是一种以获取 最短回收停顿时间为目标 的收集器。
 *      4、G1垃圾回收器（G1 Garbage Collector）
 *  2、一文读懂Java 11的ZGC为何如此高效 - Java 11包含一个全新的垃圾收集器--ZGC，它由Oracle开发，承诺在数TB的堆上具有非常低的暂停时间。
 *     - 我们继续探讨ZGC的工作原理：为了实现其目标，ZGC给Hotspot Garbage Collectors增加了两种新技术：着色指针和读屏障。
 *       1）着色指针 - 着色指针是一种将信息存储在指针（或使用Java术语引用）中的技术。因为在64位平台上（ZGC仅支持64位平台），指针可以处理更多的内存，因此可以使用一些位来存储状态。
 *                 - ZGC将限制最大支持4Tb堆（42-bits），那么会剩下22位可用，它目前使用了4位： finalizable， remap， mark0和mark1。
 *                 - 着色指针的一个问题是，当您需要取消着色时，它需要额外的工作（因为需要屏蔽信息位）。
 *       2）读屏障 - 读屏障是每当应用程序线程从堆加载引用时运行的代码片段（即访问对象上的非原生字段non-primitive field）
